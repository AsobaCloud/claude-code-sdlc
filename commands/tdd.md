---
description: Orchestrates the TDD workflow using epistemically isolated subagents. Use this skill IMMEDIATELY after plan approval for any code-change plan.
allowed-tools: Bash(~/.claude/scripts/*), Read, Write, Agent
---

# TDD Orchestration Command

This command runs the three-phase TDD cycle using isolated subagents. Each agent runs in its own context window and cannot see the other agents' reasoning or the implementation plan.
...
Use the Agent tool with `subagent_type: "general"` (tdd-test-writer not available, using general with restricted prompt). The prompt MUST contain:
...
After `/approve-tests`, use the Agent tool with `subagent_type: "general"` (tdd-implementer not available, using general with restricted prompt). The prompt MUST contain:
...
Ask the user if they want a refactoring pass. If yes, launch `tdd-refactorer` (using general agent):
